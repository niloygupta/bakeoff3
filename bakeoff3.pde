...
import java.util.Collections;
  ...
  public void draw() {
    ...
    fill(0,0,255);
    rect(width-60,0,100,180);
    fill(255,50); //TRANSPARENT
    text(">", width-60, 50);
  ...
  private boolean internalMatch()
  {
    boolean result = (isVertexNearTarget(width/2 - screenZ/2, height/2- screenZ/2)) &&
        (isVertexNearTarget(width/2 + screenZ/2, height/2- screenZ/2))&&(isVertexNearTarget(width/2 + screenZ/2, height/2 + screenZ/2))
        &&(isVertexNearTarget(width/2 - screenZ/2, height/2 + screenZ/2));
  ...
  public void mouseReleased()
  {
    ...
    //rect(width-40,0,60,100);
    if ((mouseX>=width -80 && mouseY<=180)|| internalMatch())
    {
